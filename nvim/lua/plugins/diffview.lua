return {
  {
    ...
    config = function()
      ...
      vim.keymap.set("n", "<leader>gh", "<cmd>DiffviewFileHistory %<CR>", { desc = "History of current file" })

      -- git status
      vim.keymap.set("n", "<leader>gs", "<cmd>DiffviewOpen -uno<CR>", { desc = "Git status" })
      vim.keymap.set("n", "<leader>gS", "<cmd>DiffviewOpen<CR>", { desc = "Git status with untracked files" })
